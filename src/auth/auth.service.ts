...
export class AuthService {
  ...
  async login(loginDto: LoginDto, ipAddress: string, userAgent: string) {
    try {
      ...
      const isLocked = await this.lockoutService.isAccountLocked(loginDto.email);
      
      if (isLocked) {
        const remainingTime = await this.lockoutService.getRemainingLockoutTime(loginDto.email);
      ...
      if (!user) {
        await this.lockoutService.recordFailedAttempt(loginDto.email, ipAddress);
      ...
      await this.lockoutService.recordSuccessfulAttempt(loginDto.email, ipAddress);
