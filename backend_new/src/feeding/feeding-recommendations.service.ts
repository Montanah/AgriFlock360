...
export class FeedingRecommendationsService {
  ...
  async createRecommendation(
    ...
    const overlapping = await this.recommendationRepository
      ...
      .where('rec.bird_type_id = :birdTypeId', {
        birdTypeId: createDto.bird_type_id,
      })
      .andWhere('rec.is_active = true')
      .andWhere('(rec.age_start <= :ageEnd AND rec.age_end >= :ageStart)', {
        ageStart: createDto.age_start,
        ageEnd: createDto.age_end,
      })
      .getOne();
  ...
  async getRecommendations(query: QueryRecommendationsDto) {
    ...
    if (bird_type_id) {
      queryBuilder.andWhere('rec.bird_type_id = :bird_type_id', {
        bird_type_id,
      });
    }
    ...
    if (age !== undefined) {
      queryBuilder.andWhere('rec.age_start <= :age AND rec.age_end >= :age', {
        age,
      });
    }
  ...
  async getRecommendationsByBirdType(
    birdTypeId: string,
  ): Promise<FeedingRecommendation[]> {
  ...
  async updateRecommendation(
    ...
    if (recommendation.is_system_default) {
      throw new BadRequestException(
        'Cannot modify system default recommendations',
      );
    }
    ...
    if (
      updateDto.bird_type_id &&
      updateDto.bird_type_id !== recommendation.bird_type_id
    ) {
      const birdType = await this.birdTypeRepository.findOne({
    ...
    this.logger.log(
      `Feeding recommendation updated: ${recommendation.stage_name}`,
    );
  ...
  async deleteRecommendation(id: string): Promise<void> {
    ...
    if (recommendation.is_system_default) {
      throw new BadRequestException(
        'Cannot delete system default recommendations',
      );
    }
    ...
    this.logger.log(
      `Feeding recommendation deleted: ${recommendation.stage_name}`,
    );
  }
    ...
    for (const recData of defaultRecommendations) {
      try {
        ...
        if (!existing) {
          ...
          await this.recommendationRepository.save(recommendation);
          this.logger.log(
            `Seeded: ${recData.stage_name} for ${recData.birdTypeName}`,
          );
        }
      } catch (error) {
        this.logger.error(
          `Failed to seed recommendation: ${recData.stage_name}`,
          error,
        );
      }
    }
    ...
    return [
      ...
      },
      {
        ...
        recommendation: {
          ...
          quantity_per_bird_per_day: 0.12,
          times_per_day: 3,
          feeding_times: { slots: ['06:00', '12:00', '18:00'] },
          ...
          metadata: {
            energy_level: 'High',
            special_requirements: [
              'Feed 24/7 if under lights',
              'Target 2-2.5kg live weight',
            ],
          },
        },
      },
      ...
      },
      {
        ...
        recommendation: {
          ...
          supplements:
            'Extra calcium (crushed shells or limestone) for strong eggshells',
          metadata: {
            calcium_percentage: 3.5,
            special_requirements: [
              'Calcium crucial for shells',
      ...
      // KIENYEJI/INDIGENOUS
      {
        ...
        recommendation: {
          ...
          feed_type: 'Starter mash',
          protein_percentage: 18.0,
          quantity_per_bird_per_day: 0.03,
          times_per_day: 2,
      ...
      },
      {
        ...
        recommendation: {
          ...
          feed_type: 'Grower mash',
          protein_percentage: 16.0,
          quantity_per_bird_per_day: 0.06,
          times_per_day: 2,
          feeding_times: { slots: ['07:00', '17:00'] },
      ...
      },
      {
        ...
        recommendation: {
          ...
          protein_percentage: 16.0,
          quantity_per_bird_per_day: 0.08,
          times_per_day: 2,
          feeding_times: { slots: ['07:00', '17:00'] },
          ...
          metadata: {
            special_requirements: [
              'Free range beneficial',
              'Natural foraging supplements feed',
            ],
          },
        },
      },
      // DUCKS
      {
        ...
        recommendation: {
          ...
          quantity_per_bird_per_day: 0.04,
          times_per_day: 3,
          feeding_times: { slots: ['06:00', '12:00', '18:00'] },
      ...
      },
      {
        ...
        recommendation: {
          ...
          protein_percentage: 16.0,
          quantity_per_bird_per_day: 0.18,
          times_per_day: 3,
          feeding_times: { slots: ['06:00', '12:00', '18:00'] },
