...
export class VaccineCatalogService {
  ...
  async getVaccines(query: QueryVaccinesDto) {
    ...
    if (vaccine_type) {
      queryBuilder.andWhere('vaccine.vaccine_type = :vaccine_type', {
        vaccine_type,
      });
    }
    ...
    if (bird_type) {
      queryBuilder.andWhere(
        '(vaccine.bird_type = :bird_type OR vaccine.bird_type = :all)',
        {
          bird_type,
          all: 'all',
        },
    ...
    if (is_recommended !== undefined) {
      queryBuilder.andWhere('vaccine.is_recommended = :is_recommended', {
        is_recommended,
      });
    }
  ...
  ): Promise<VaccineCatalog> {
    const vaccine = await this.getVaccine(id);

    // Check if new name conflicts
    if (
      updateDto.vaccine_name &&
      updateDto.vaccine_name !== vaccine.vaccine_name
    ) {
      const existing = await this.vaccineCatalogRepository.findOne({
  ...
  async deleteVaccine(id: string): Promise<void> {
    ...
    if (vaccine.vaccinations && vaccine.vaccinations.length > 0) {
      ...
      await this.vaccineCatalogRepository.save(vaccine);
      this.logger.log(
        `Vaccine deactivated (has usage history): ${vaccine.vaccine_name}`,
      );
    } else {
    ...
    const commonVaccines: CreateVaccineDto[] = [
      {
        ...
        vaccine_type: 'viral',
        description:
          'Live attenuated vaccine against Newcastle Disease, one of the most important poultry diseases',
        ...
        is_recommended: true,
        usage_instructions:
          'Administer one drop in the eye. Ensure all birds receive the vaccine.',
      ...
      },
      {
        ...
        vaccine_type: 'viral',
        description:
          "Protects against Marek's disease, a highly contagious viral disease",
        ...
        is_recommended: true,
        usage_instructions:
          'Administer subcutaneously at day old in the hatchery',
      ...
      },
      {
        ...
        vaccine_type: 'viral',
        description:
          'Protects against Infectious Bronchitis virus affecting respiratory system',
      ...
      },
      {
        ...
        is_recommended: true,
        usage_instructions:
          'Dissolve in chlorine-free drinking water. Ensure all birds drink within 2 hours.',
        storage_conditions: '2-8°C',
        withdrawal_period_days: 0,
      },
      {
...

      {
        ...
        is_recommended: true,
        usage_instructions:
          'Administer via intramuscular injection at hatchery',
        storage_conditions: '2-8°C',
        withdrawal_period_days: 0,
      },
      {
        ...
        vaccine_type: 'viral',
        description:
          'Protects against Newcastle Disease and Infectious Bronchitis',
    ...
    for (const vaccine of commonVaccines) {
      try {
        ...
        if (error instanceof ConflictException) {
          ...
          this.logger.error(
            `Failed to seed vaccine: ${vaccine.vaccine_name}`,
            error,
          );
        }
      }
