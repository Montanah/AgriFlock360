...
export class HousingQuotationsService {
  ...
  async generateQuotation(
    ...
    const { bird_capacity, labor_percentage = 26, notes, metadata } = generateDto;

    // Find exact match or closest capacity
    const availableCapacities = await this.getAvailableCapacities();
    const closestCapacity = this.findClosestCapacity(bird_capacity, availableCapacities);

    if (!closestCapacity) {
      throw new BadRequestException('No housing data available for this capacity');
    }
    ...
    const materialsSubtotal = materials.reduce((sum, m) => sum + m.total_cost, 0);
  ...
  async getQuotation(quotationId: string, userId: string): Promise<HousingQuotation> {
  ...
  async getUserQuotations(userId: string, page: number = 1, limit: number = 20) {
  ...
  async getQuotationSummary(userId: string) {
    ...
    const totalValue = quotations.reduce((sum, q) => sum + parseFloat(q.grand_total.toString()), 0);
  ...
  private findClosestCapacity(target: number, available: number[]): number | null {
